import 'package:flutter/material.dart';

class TutorialKeys {
  static final GlobalKey missionsKey = GlobalKey();
  static final GlobalKey classesKey = GlobalKey();
  static final GlobalKey profileKey = GlobalKey();
  static final GlobalKey socialTabKey = GlobalKey();
  static final GlobalKey socialTabActiveKey = GlobalKey();
  static final GlobalKey techniquesTabKey = GlobalKey();
  static final GlobalKey techniquesTabActiveKey = GlobalKey();
  static final GlobalKey addTrainingFabKey = GlobalKey();
}
